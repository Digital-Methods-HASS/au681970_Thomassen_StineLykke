...
```{r load-data}
Western_Europe <- read.csv("data/homicide-rates-across-western-europe.csv")
```
...
```{r eval = TRUE}
monarchs2 <- read_csv2("data/raw_data_kings.csv")

monarchs2 %>% mutate(duration=Regeringsperiode_slut-Regeringsperiode_start) %>% 
  mutate(midyear=Regeringsperiode_slut-duration/2) %>% 
  ggplot(aes(x=midyear,y=duration)) +
  geom_point(color="purple") + 
  geom_smooth(color="black")+
  labs(title = "Reign duration among Danish rulers ca 900 - today",
       x = "midyear",
       y = "duration")
```
...
Det er gjort via fremgangsmåden nedenfor og er illustreret på plottet længere oppe


ggplot(data = Western_Europe, aes(x = Year, y = homicides_per_100k, color = Entity)) + 
  geom_line() +
  labs(title = "Long-term Trend in Homicides per 100k in Western Europe",
       x = "Year",
       y = "Homicides_per_100k")+
  theme_minimal() +
  facet_wrap(~ Entity, ncol = 2)+
...
Jeg har lavet to datavisualiseringer, der viser dels den langvarige tendens i drab per 100.000 mennesker i lande i Vesteuropa og dels danske regenters regeringsperiode på tronen fra ca. 900 og frem til i dag. I forhold til drab i Vesteuropa, så er det fælles for alle landene, at der bliver begået færre drab, og tendensen er klart faldende. Det tegner et billede af, at mennesker i langt mindre grad dræber andre mennesker, hvilket må siges at være en mere civiliseret tilgang at agere på. En mulig grund kan være forbedrede levevilkår for det enkelte menneske, hvor man i mindre grad er drevet ud i kriminalitet. I forhold til regeringsperioderne for danske regenter, så har den gennemgået en lettere stigende tendens. En begrundelse for det kan være, at magtrivaliseringer og kongemord før i tiden gjorde det farligt at sidde på tronen sammenlignet med i dag, og dermed forkortede visse kongers regeringsperiode, fordi de blev dræbt (Fx Erik Klipping i 1286). Derudover har den stigende levestandard også kunne gøre, at regenter har kunne leve længere og derved har kunne regere længere tid. Derfor vil jeg konkludere, at vi er mere civiliserede i dag, fordi mennesker i mindre grad dræber andre mennesker.   
